...
def _get_financial_report_sina(
    ...
    if isinstance(items, list) and items:
        # Legacy response used by older Sina nodes.
        df = pd.DataFrame(items)
    else:
        # Current FinanceReport2022 response groups statement line items by
        # report date under report_list. Flatten each report into one row so
        # the public function keeps returning the same tabular contract.
        reports = result.get("report_list", {})
        rows = []
        if isinstance(reports, dict):
            for report_date, report in reports.items():
                if not isinstance(report, dict):
                    continue
                raw_date = str(report_date)
                formatted_date = (
                ...
                row = {
                    "报告日": formatted_date,
                    "报告类型": report.get("rType", ""),
                    "币种": report.get("rCurrency", ""),
                    "审计状态": report.get("is_audit", ""),
                    "_报告周期": next(
                        (
                            entry.get("date_type")
                            for entry in result.get("report_date", [])
                            if str(entry.get("date_value")) == raw_date
                        ),
                        None,
                    ),
                }
                for entry in report.get("data", []):
                    if not isinstance(entry, dict):
                        continue
                    title = str(entry.get("item_title", "")).strip()
                    value = entry.get("item_value")
    ...
    if freq.lower() == "annual" and "报告日" in df.columns:
        if "_报告周期" in df.columns:
            df = df[df["_报告周期"] == 4]
        else:
            months = pd.to_datetime(df["报告日"], errors="coerce").dt.month
            df = df[months == 12]

    return df.drop(columns=["_报告周期"], errors="ignore").head(8)
...
def get_insider_transactions(
    ...
    try:
        ...
        category_note = "股东研究"
        # mootdx normally returns a string when the named F10 category is
        # found, but some TDX nodes return a {category: text} mapping instead.
        # Preserve the useful response rather than crashing on dict.strip().
        if isinstance(text, dict):
            named = text.get("股东研究")
            if named:
                text = named
            else:
                category_note = " / ".join(str(key) for key in text.keys()) or "fallback"
                text = "\n\n".join(str(value) for value in text.values() if value)
        ...
        header += f"# F10 category: {category_note}\n"
        header += (
            f"# Data retrieved on: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
        )
